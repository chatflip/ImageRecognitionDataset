...
def make_subset(src):
    ...
    for num_subset in range(10):
        ...
        train_lists = np.genfromtxt("csv/caltech101_train_subset{}.csv"
                                    .format(num_subset), dtype=np.str)
        test_lists = np.genfromtxt("csv/caltech101_test_subset{}.csv"
                                   .format(num_subset), dtype=np.str)
...
if __name__ == '__main__':
    ...
    tar_filename = '101_ObjectCategories.tar.gz'
    print('Downloading: {}'.format(tar_filename))
    download(baseurl, tar_filename)
    untar(tar_filename)
    # Create subset
    print('Create subset: 101_ObjectCategories')
    make_subset('101_ObjectCategories')
